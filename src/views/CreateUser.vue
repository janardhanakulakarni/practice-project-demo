  ...
  <div>
    ...
    >
      <v-card>
        ...
        <v-card-text
          >By selecting any option, with respect to that the user/account will
          get created.</v-card-text
        >
        <v-radio-group v-model="selectedUserType" column class="ml-5">
          <v-radio label="Admin" color="primary" value="1"></v-radio>
          <v-radio
            label="HOD (Head of the Dept)"
            color="primary"
            value="2"
          ></v-radio>
          <v-radio label="Teaching Staff" color="primary" value="3"></v-radio>
          <v-radio label="Accountant" color="primary" value="4"></v-radio>
          <v-radio label="Hostel HM" color="primary" value="5"></v-radio>
        </v-radio-group>
        <v-card-text v-if="typeNotSelected" class="mt-n3"
          ><span class="err-text font-weight-bold"
            >Please select atleast one option</span
          ></v-card-text
        >
        ...
        <v-spacer></v-spacer>
        <v-card-actions>
          <v-btn color="red darken-1" text @click="showSelectionDailog = false">
            Cancel
          </v-btn>
          <v-btn color="green darken-1" text @click="onSelectType">
            Continue
          </v-btn>
        </v-card-actions>
      </v-card>
      ...
      >
        ...
        <v-tab-item>
          <user-contact-details
            @onClickFinish="createUserWithDetails"
...
export default {
  ...
  data: () => ({
    ...
    selectedTab: 0,
    tabItems: ["General Details", "Contact Details"],
    msg: {
      color: "",
      desc: "",
    },
    ...
    async getAllDropDownVals() {
      ...
      arrList.forEach((item) => {
        if (item === "dept") this.setDropDown(data.data.department, "dept");
          ...
          this.setDropDown(data.data.maritalStatus, "maritalStatus");
        else if (item === "quali")
          this.setDropDown(data.data.qualification, "quali");
